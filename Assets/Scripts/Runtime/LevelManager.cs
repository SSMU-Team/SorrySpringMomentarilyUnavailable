...
namespace SceneManagement
{
	...
	public struct Level
	{
		...
		public List<SceneReference> levels_exclude;
	}
...
#pragma warning restore 0649
		...
		private void Start()
		{
			...
			{
				g.levels.AddRange(m_sharedLevel.levels);
		...
		{
			foreach(SceneReference l in level.levels_exclude)
			{
				UnloadSceneLoaded(l);
			}
			...
			foreach(Level g in m_games)
			{
				foreach(SceneReference l in g.levels)
				{
					UnloadSceneLoaded(l);
				}
			}

			foreach(SceneReference l in level.levels)
			{
				LoadSceneUnloaded(l);
			}
		}

		private void UnloadSceneLoaded(SceneReference path)
		{
		...
		private void LoadSceneUnloaded(SceneReference path)
		{
			if(!SceneManager.GetSceneByPath(path).isLoaded)
			{
