...
namespace Hierarchy2
{
	internal class HierarchySettings : ScriptableObject
	{
		...
		{
			SettingsProvider provider = new SettingsProvider("Project/Hierarchy", SettingsScope.Project)
			{
				label = "Hierarchy",
				...
				activateHandler = (searchContext, rootElement) =>
				{
					...
					HierarchySettings settings = editor.target as HierarchySettings;
					...
					Toggle displayCustomObjectIcon = new Toggle("Display Custom Icon")
					{
					...
					Toggle displayRowBackground = new Toggle("Display RowBackground")
					{
					...
					Toggle displayTreeView = new Toggle("Display TreeView")
					{
					...
					Toggle displayGrid = new Toggle("Display Grid")
					{
					...
					Toggle displayComponents = new Toggle("Display Components Icon")
					{
					...
					TextField componentListInput = new TextField("Components")
					{
					...
					ComponentSize componentSizeEnum = ComponentSize.Normal;
					...
					Toggle applyTagTargetAndChild = new Toggle("Tag Recursive Change")
					{
					...
					Toggle displayLayer = new Toggle("Display Layer")
					{
					...
					Toggle applyLayerTargetAndChild = new Toggle("Layer Recursive Change")
					{
					...
					{
						label = "Header Default Tag",
						value = settings.headerDefaultTag
					};
					...
					Toggle onlyDisplayWhileMouseHovering = new Toggle("Display Hovering")
					{
		...
		{
			string[] guids = AssetDatabase.FindAssets(string.Format("t:{0}", typeof(HierarchySettings).Name));

			if(guids.Length > 0)
			{
				HierarchySettings asset = AssetDatabase.LoadAssetAtPath<HierarchySettings>(AssetDatabase.GUIDToAssetPath(guids[0]));
				if(asset != null)
				{
					return asset;
				}
	...
	internal class SettingsInspector : Editor
	{
		...
		{
			settings = target as HierarchySettings;
		}
