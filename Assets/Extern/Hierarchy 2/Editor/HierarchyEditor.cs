...
namespace Hierarchy2
{
	...
	public sealed class HierarchyEditor
	{
		...
		private Dictionary<int, HierarchyCanvas> hierarchyCanvas = new Dictionary<int, HierarchyCanvas>();
		private Dictionary<int, UnityEngine.Object> selectedComponents = new Dictionary<int, UnityEngine.Object>();
		private Dictionary<string, string> dicComponents = new Dictionary<string, string>(StringComparer.Ordinal);
		private UnityEngine.Object activeComponent;
		private GUIContent tooltipContent = new GUIContent();
		private const string componentDefaultTooltip = "\n\nM2-click: Edit\nM3-click: Instant Inspector";
		private const string customIconDefaultTooltip = "M2-click: To customize";
		...
		private bool selectionStyleAfterInvoke = false;
		private bool checkingAllHierarchy = false;
		private Event currentEvent;
		private RowItem rowItem = new RowItem();
		private RowItem previousElement = null;
		...
		private static List<Type> InternalEditorType = new List<Type>();
		private static Dictionary<string, Type> dicInternalEditorType = new Dictionary<string, Type>();
		...
		private static Action<Rect, UnityEngine.Object, int> DisplayObjectContextMenuDelegate;
		...
		private static void InternalReflection()
		{
			...
			Type IconSelector = typeof(EditorWindow).Assembly.GetTypes().Single(type =>
				type.BaseType == typeof(EditorWindow) && type.Name == nameof(IconSelector));
		...
		private void DelayCall()
		{
			if(checkingAllHierarchy == true)
			{
				...
				foreach(EditorWindow window in GetAllSceneHierarchyWindowsDelegate())
				{
					...
					{
						HierarchyWindow hierarchyWindow = new HierarchyWindow(window);
		...
		private bool hierarchyChangedRequireUpdating = false;
		...
		private bool prefabStageChanged = false;
		...
		private void CreateCanvas(EditorWindow hierarchyWindow)
		{
			...
			VisualElement root = hierarchyWindow.rootVisualElement;
			HierarchyCanvas canvas = new HierarchyCanvas
			{
				name = nameof(HierarchyCanvas)
			};
			root.Add(canvas);
		...
		{
			foreach(KeyValuePair<int, HierarchyCanvas> canvas in hierarchyCanvas)
			{
		...
		{
			if(HierarchyLocalData.GetInstance(scene, out HierarchyLocalData hierarchyLocalData))
			{
		...
		private HierarchyLocalData CreateHierarchyLocalData(Scene scene)
		{
		...
		private void HierarchyOnGUI(int selectionID, Rect selectionRect)
		{
			...
			else
			{
				...
				bool hasHierarchyLocalData =
					HierarchyLocalData.instances.TryGetValue(rowItem.gameObject.scene, out HierarchyLocalData hld);
		...
		private void CustomRow()
		{
			...
			{
				Rect rect = rowItem.nameRect;
				rect.center += rowItem.customRowItem.labelOffset;

				Color guiColor = GUI.color;
				Color color = rowItem.customRowItem.labelColor;
		...
		private void ElementAsHeader()
		{
			...
			GUIContent content = new GUIContent(rowItem.name.Remove(0, settings.headerPrefix.Length));
		...
		{
			List<UnityEngine.Object> components = rowItem.gameObject.GetComponents(typeof(Component)).ToList<UnityEngine.Object>();
			...
			for(int i = 0; i < length; ++i)
			{
				...
				try
				{
					...
					{
						bool isMono = false;
						if(comType.BaseType == typeof(MonoBehaviour))
						{
							isMono = true;
						}
		...
		private void ComponentIcon(UnityEngine.Object component, Type componentType, Rect rect, bool isMaterial = false)
		{
			...
			if(rect.Contains(currentEvent.mousePosition))
			{
					...
					if(currentEvent.button == 1)
					{
						if(currentEvent.control)
						{
							...
							componentGenericMenu.AddItem(new GUIContent("Remove All Component"), false, () =>
							{
								...
								foreach(KeyValuePair<int, UnityEngine.Object> selectedComponent in selectedComponents.ToList())
								{
				...
				if(currentEvent.type == EventType.MouseUp)
				{
					if(currentEvent.button == 2)
					{
						...
						foreach(KeyValuePair<int, UnityEngine.Object> selectedComponent in selectedComponents)
						{
		...
		private GUIContent tmpSceneContent = new GUIContent();
		...
		private sealed class HierarchyWindow
		{
			...
			public int instanceID = int.MinValue;
		...
		internal sealed class MenuCommand
		{
			...
			{
				new GameObject(string.Format("{0}Header", HierarchyEditor.instance.settings.headerPrefix));
			}
		}
	}
}