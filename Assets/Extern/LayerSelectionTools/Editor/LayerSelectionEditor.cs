...
namespace gbFactory.Tools
{
	...
	public class LayerSelectionEditor : EditorWindow
	{
		...
		private void OnGUI()
		{
			...
			foreach(LayerSelected l in Layers.Values)
			{
				LayersEditor(l);
			}
		...
		private void GroupSettingsEditor()
		{
			if(foldOut = EditorGUILayout.Foldout(foldOut, "Group settings"))
			{
				...
				if(selectAll = EditorGUILayout.Toggle("Select all layers", selectAll))
				{ Save(); }
				...
				if(iconsVisible = EditorGUILayout.Toggle("Show all icons", iconsVisible))
				{ Save(); }
		...
		private void LayersEditor(LayerSelected layer)
		{
			...
			{
				fontStyle = layer.enabled ? FontStyle.Bold : FontStyle.Normal,
			...
			if(layer.enabled = EditorGUILayout.Toggle(layer.enabled, GUILayout.Width(50)))
			{ Save(); }
		...
		{
			foreach(LayerSelected s in Layers.Values)
			{
				s.enabled = select;
			}
		}
		...
		private void Traverse(GameObject obj, ref List<GameObject> all)
		{
		...
		private void Select()
		{
			...
			List<GameObject> list = new List<GameObject>(allObjectsInScene.Count);
			foreach(LayerSelected l in Layers.Values)
			{
				if(l.enabled)
				{
		...
		private void Load()
		{
			...
			try
			{
				...
				foreach(string s in UnityEditorInternal.InternalEditorUtility.layers)
				{
	...
	internal class HierarchyIcon
	{
		...
		private static void AddIcon(int instanceID, Rect selectionRect)
		{
			GameObject instaceIDGameObject = EditorUtility.InstanceIDToObject(instanceID) as GameObject;
			if(instaceIDGameObject == null)
			{
				return;
			}

			int layer = instaceIDGameObject.layer;
			string layerName = LayerMask.LayerToName(instaceIDGameObject.layer);
			if(LayerSelectionEditor.Layers.ContainsKey(layerName))
			{
				if(!LayerSelectionEditor.Layers[layerName].activeIcon)
				{
					return;
				}
				...
				if(GUI.Button(r, texture, GUIStyle.none))
				{
					...
					{
						GameObject obj = (EditorUtility.InstanceIDToObject(objectWithLayers[i]) as GameObject);
						if(obj.layer == layer)
						{
							selected.Add(obj);
						}
